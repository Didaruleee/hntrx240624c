...
const SalesHistoryTable = ({
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        if (
          collectionName &&
          collectionName !== salesById[TAB_TYPES.GLOBAL].id
        ) {
          const globalSales = await getSalesHistoryForCollection(
            collectionName,
            1
          );
  ...
  const fetchNextGlobalPage = async () => {
    ...
    const globalSales = await getSalesHistoryForTemplate(collectionName, page);
