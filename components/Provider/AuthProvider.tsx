...
export const AuthProvider = ({ children }: Props): JSX.Element => {
  ...
  const [currentUserXprBalance, setCurrentUserXprBalance] = useState<string>('');
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && !currentUser) {
      ...
      if (cachedUser) {
        const {acc, actor, permission } = JSON.parse(cachedUser);
        setCurrentUser({
          actor,
          acc,
          permission,
  ...
  const updateCurrentUserBalance = async (chainAccount: string) => {
    ...
    const xprBalance = await proton.getAccountXprBalance(chainAccount);
    setCurrentUserXprBalance(xprBalance);
  };
