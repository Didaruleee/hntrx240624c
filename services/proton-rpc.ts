...
class ProtonJs {
  ...
  getAccountXprBalance = async (chainAccount: string): Promise<string> => {
    const balance = await this.rpc.get_currency_balance(
      'eosio.token',
      chainAccount,
      'XPR',
    );
    const price = balance.length ? balance[0] : `${0} XPR`;
    return price;
  };
  ...
  getXPRrate = async (): Promise<number> => {
    try {
      const res = await this.rpc.get_table_rows({
        code: 'proton.swaps',
        scope: 'proton.swaps',
  ...
  getAccountRam = async (
    ...
    try {
      const account:any = await this.rpc.get_account(account_name);
